						...
						<div class="panel-body">
							...
							<form action = "UpdateBookServlet" enctype="multipart/form-data" method = "post">
								<div class="top-margin">
									...
									<p><%=request.getParameter("bookid") %><input type = "hidden" name = "bookid"  value = '<%=request.getParameter("bookid")%>'></p>
								</div>
								<div class="top-margin">
									<label>书本名称 - Book Name <span class="text-danger">*</span></label>
									<input type = "text" name = "bookname"  class="form-control" value = '<%=request.getParameter("bookname")%>' placeholder = "在此输入书本名称">
								</div>
								<div class="top-margin">
									<label>作者 - Author <span class="text-danger">*</span></label>
									<input type = "text" name = "author"  class="form-control" value = '<%=request.getParameter("author")%>' placeholder = "在此输入作者信息">
								</div>
								<div class="top-margin">
									<label>库存 - Amont <span class="text-danger">*</span></label>
									<input type = "text" name = "amont"  class="form-control" value = '<%=request.getParameter("amont")%>' placeholder = "在此输入库存">
								</div>
								<div class="top-margin">
									<label>上传图片 - Picture </label>
								...
								<div class="row">
									<div class="col-lg-8">
										<p class="text-danger"><%if(request.getAttribute("error")!=null){ %><%=request.getAttribute("error")%><% } %></p>
										<b><a href="manager/librarymanage.jsp">返回</a></b>
									</div>
					...
					<table>
						...
						<tr><td>
							<form action = "UpdateBookServlet" enctype="multipart/form-data" method = "post">
								...
								索引号<p><%=request.getParameter("bookid") %><input type = "hidden" name = "bookid"  value = '<%=request.getParameter("bookid")%>'></p>
								<p>名称&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<input type = "text" name = "bookname" class = "aboutbook"placeholder = "在此输入书本名称"  value = '<%=request.getParameter("bookname")%>'>
								</p>
								<p>作者&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<input type = "text" name = "author" class = "aboutbook"placeholder = "在此输入作者信息" value = '<%=request.getParameter("author")%>'>
								</p>
								<p>库存&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<input type = "text" name = "amont" class = "aboutbook"placeholder = "在此输入库存"value = '<%=request.getParameter("amont")%>'>
								</p>
										上传图片
